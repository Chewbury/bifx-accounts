...
# In the format:
# Username, user id, group id, sudo access
...
with open('accounts.txt', 'rt') as fin:
    cin = csv.DictReader(fin, fieldnames=['uname', 'uid', 'gid', 'sudo'])
...
for user in accounts:
    ...
    print("!auth.extrapassword\t mADD(" + user.get('uname') + ")")
    print("auth.pw_uid_" + user.get('uname') + " \t" + user.get('uid'))
    print("auth.pw_gid_" + user.get('uname') + " \t" + user.get('gid'))
    print("auth.pw_dir_" + user.get('uname') + " \t" + "/homes/" + user.get('uname'))
